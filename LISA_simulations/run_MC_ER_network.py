...
import networkx as nx, itertools, scipy, time, \
                os, pickle, sys, json, multiprocessing as mp
import itertools
import numpy as np
...
if __name__ == '__main__':
    ...
    """
    with open(targetDirectory + '/modelSettings.json') as f:
        modelSettings = json.load(f)
    ...
    with open(targetDirectory + '/corrTimeSettings.json') as f:
        corrTimeSettings = json.load(f)
    mixingTime = corrTimeSettings['burninSteps']
    ...
    np.save(f'{targetDirectory}/MI_pairwise_all_T={model.t}.npy', MI)
    """
